...
DEFAULT_DATA_ROOT = os.path.normpath(
    os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'cifar-100-python')
)

class Cifar100Imbanlance(Dataset):
    def __init__(self, imbanlance_rate=0.1, file_path=DEFAULT_DATA_ROOT, num_cls=100, transform=None,
                 train=True):
        ...
        self.classes = torchvision.datasets.CIFAR100(
            root=os.path.dirname(os.path.normpath(file_path)),
            train=train,
            download=True
        ).classes
