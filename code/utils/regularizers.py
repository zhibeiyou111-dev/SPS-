...
class MaxNorm_via_PGD():
    ...
    def setPerLayerThresh(self, model):
        ...
        # --- FIX ---
        # Access the actual model using .module if it's a DataParallel object
        actual_model = model.module if isinstance(model, torch.nn.DataParallel) else model
        
        # Now use actual_model to get the layers
        for curLayer in [actual_model.fc_cb.weight, actual_model.fc_cb.bias]:
    ...
    def PGD(self, model):
        ...
        # --- FIX ---
        # Also access the actual model here
        actual_model = model.module if isinstance(model, torch.nn.DataParallel) else model
        
        # And use it to access the layers
        for i, curLayer in enumerate([actual_model.fc_cb.weight, actual_model.fc_cb.bias]):
            curparam = curLayer.data
            curparam_vec = curparam.reshape((curparam.shape[0], -1))
            ...
            idx = idx.squeeze()
            if idx.any(): # Check if there are any neurons to scale
                tmp = curLayerThresh / (neuronNorm_curparam[idx].squeeze())**(self.tau)
                for _ in range(len(scalingVect.shape) - 1):
                    tmp = tmp.unsqueeze(-1)
                
                # Ensure tmp and scalingVect[idx] are broadcastable
