...
def get_dataset(args):
    ...
    if args.dataset == 'ImageNet-LT':
        trainset = dataset_lt_data.LT_Dataset(args.root, '/home/liuyang/project/long-tail/GLMC-CVPR2023/data/data_txt/ImageNet_LT_train.txt',util.TwoCropTransform(transform_train),'/home/liuyang/project/long-tail/GLMC-CVPR2023/data/data_txt/ImageNetlabels.txt')
        testset = dataset_lt_data.LT_Dataset(args.root, '/home/liuyang/project/long-tail/GLMC-CVPR2023/data/data_txt/ImageNet_LT_test.txt',transform_val,'/home/liuyang/project/long-tail/GLMC-CVPR2023/data/data_txt/ImageNetlabels.txt')
        return trainset,testset
...
def main_worker(gpu, args):
    ...
    print(args.gpu)
    trainer = Trainer(args, model=model,train_loader=train_loader, val_loader=val_loader,weighted_train_loader=weighted_train_loader, per_class_num=train_cls_num_list,log=logging,class_names = class_names)
    trainer.train()
    end_time = time.time()
...
if __name__ == '__main__':
    ...
    # Resolve the bundled dataset relative to this file.
    parser.add_argument('--root', type=str, default=DEFAULT_DATA_ROOT, help="dataset setting")
    ...
    parser.add_argument('--contrast_weight', default=2,type=int,help='Mixture Consistency  Weights')
    # etc.
    ...
    parser.add_argument('--root_log', type=str, default='output_trainer1105_ht100_interactive/')
    parser.add_argument('--root_model', type=str, default='output_trainer1105_ht100_interactive/')
    parser.add_argument('--store_name', type=str, default='GLMC-CVPR2023/output/')
    # --- In main.py ---
    parser.add_argument('--fusion_gamma', default=10, type=float, help='Weight for semantic low-rank fusion loss')
    parser.add_argument('--fusion_rank_k', default=16, type=int, help='Rank K for SVD in fusion loss')
    parser.add_argument('--fusion_num_heads', default=5, type=int, help='Number of similar head classes K for fusion')
    parser.add_argument('--fusion_svd_freq', default=5, type=int, help='Frequency (in epochs) to update SVD basis')
    parser.add_argument('--fusion_warmup', default=20, type=int, help='Epochs before starting fusion loss calculation')
    parser.add_argument('--head_threshold', default=100, type=int, help='Sample count threshold to define head classes')
    main()
